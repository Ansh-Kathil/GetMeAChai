...
const Dashboard = () => {
    ...
    const getData = async () => {
        ...
        if (u) {
            ...
            setform({ ...defaultForm, ...u });
        } else {
            // If user not found, use session for username/email
            setform({
                ...defaultForm,
    ...
    const handleSubmit = async (e) => {
        const oldUsername = session.user.name; // Store old username for comparison
        const a = await updateProfile(e, oldUsername)
        toast('Profile Updated', { /* ... */ });

            if (form.username !== oldUsername) {
                await update(); // Refresh session with new username
            }
            

        let updatedUser = await fetchuser(form.username);
        setform({
